...
void parse_param_list(PARAM_LIST *param_list, DECL **decl_list, DECL **extra_decl_list) {
    ...
    *extra_decl_list = NULL;
    ...
    while (param_list) {
        ...
        if ((token_class != RESERVED) || (decl_token->token_type != DECLARE)) {
        ...
        while (el_ptr) {
            ...
            for (var_ptr = el_ptr->name_list; var_ptr;) {
                ...
                last_param = NULL;
                for (param_ptr = param_list; param_ptr; param_ptr = param_ptr->next_param) {
                    if (!strcmp(param_ptr->param.token_name, var_ptr->name->token_name)) {
                        break;
                    } else {
                ...
                if (param_ptr) {
                    ...
                    if (extra_el_ptr == NULL) {
                        ...
                        if (extra_list_ptr) {
                            ...
                            extra_decl->decl_token = list->decl_token;
...
int parse_function(TOKEN *token) {
    ...
    out_white_space(token);
    left_shift  = !strcmp(token->token_name, "shl") || !strcmp(token->token_name, "SHL");
    right_shift = !strcmp(token->token_name, "shr") || !strcmp(token->token_name, "SHR");
...
int parse_expression(TOKEN *token) {
    ...
    while (1) {
        switch (token_class) {
            ...
            case OPERATOR:
                ...
                if (token->token_type == EQUAL) {
                    ...
                    if ((token->token_type == AT_OP) || (token->token_type == PERIOD)) {
                        token_class = get_token(token);
                        if (token_class == IDENTIFIER) {
                            /* Make it a '&' */
                            out_char('&');
                            ...
                            if (find_symbol(token, &id_type, &id_id) && (id_type->type->token_type != PROCEDURE)) {
                                /* Variable - parse it */
                            ...
                            if ((i >= 4) && (!strcmp(string_const + i - 4, "\\000"))) {
        ...
        token_class = (last_class == IDENTIFIER) ? temp_class : get_token(token);
    }
}
...
void parse_do(TOKEN *first_token) {
    ...
    switch (token_class) {
        ...
        case IDENTIFIER:
            ...
            out_token(&token);
            token_class = parse_expression(&token);
            if ((token_class != RESERVED) || (token.token_type != TO)) {
            ...
            /* Parse increment */
            if ((token_class == RESERVED) && (token.token_type == BY)) {
                /* Send <ident> += <step> */
                out_str(var_string);
                out_str(" +=");
                token_class = parse_expression(&token);
            ...
            switch (token.token_type) {
                case CASE:
                    ...
                    while (1) {
                        ...
                        (void)sprintf(case_statement, "case %d :", case_line++);
...
void parse_if(TOKEN *first_token) {
    ...
    if ((parse_expression(&token) != RESERVED) || (token.token_type != THEN)) {
...
void parse_identifier(TOKEN *first_token) {
    ...
    if (token_class == LABEL) {
        ...
        if ((next_token_class == RESERVED) && (next_token.token_type == PROCEDURE)) {
            /*
            ...
            got_type = (token_class == RESERVED) && (attrib_token.token_type >= BYTE) && (attrib_token.token_type <= SELECTOR);
            if (got_type) {
                /*
                 *	Process [ <type> ]
                 */
            ...
            while (token_class == RESERVED) {
                if (attrib_token.token_type == INTERRUPT) {
                    ...
                    if (attrib_token.token_type == EXTERNAL) {
                        ...
                        if ((attrib_token.token_type == PUBLIC) || (attrib_token.token_type == REENTRANT)) {
                        do {
