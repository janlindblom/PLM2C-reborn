...
BOOLEAN find_member_symbol(TOKEN *symbol, DECL_MEMBER *decl_ptr, DECL_ID **decl_id) {
    ...
    for (var_ptr = decl_ptr->name_list; var_ptr; var_ptr = var_ptr->next_var) {
...
BOOLEAN find_list_symbol(TOKEN *symbol, DECL_MEMBER *decl_ptr, DECL_MEMBER **decl_found, DECL_ID **decl_id) {
    for (*decl_found = decl_ptr; *decl_found; *decl_found = (*decl_found)->next_member) {
...
BOOLEAN find_symbol(TOKEN *symbol, DECL_MEMBER **decl_found, DECL_ID **decl_id) {
    ...
    for (context_ptr = context_head; context_ptr; context_ptr = context_ptr->next_context) {
        if (find_list_symbol(symbol, context_ptr->decl_head, decl_found, decl_id)) {
            return TRUE;
        }
    }
    return FALSE;
