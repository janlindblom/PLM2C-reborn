...
int get_token(TOKEN *token) {
    ...
    if (is_a_char(token_ch)) {
        ...
        *token_name_ptr     = '\0';
        ...
        for (word_ptr = &reserved_words[0]; word_ptr->token != END_OF_FILE; word_ptr++) {
        ...
        /* Check for a literal */
        if (!parsing_literal && find_symbol(token, &decl_ptr, &decl_id) && (decl_ptr->type->token_type == LITERALLY)) {
#ifdef CONVERT_CASE
            /* Convert case of literal */
            for (cvt_ptr = token->token_name; *cvt_ptr; cvt_ptr++) {
                if (is_a_uc_char(*cvt_ptr)) {
                    *cvt_ptr += 32;
                } else if (is_a_lc_char(*cvt_ptr)) {
                    *cvt_ptr -= 32;
                }
            }
#endif
