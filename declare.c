...
void find_right_paren() {
    ...
    do {
        ...
        if (token_class == LEFT_PAREN) {
            paren_count++;
        } else if (token_class == RIGHT_PAREN) {
            paren_count--;
        }
    } while (paren_count);
}
...
int get_var_list(DECL_ID **list_ptr, TOKEN *sep_token) {
    ...
    *list_ptr = NULL;
    ...
    while (1) {
        ...
        if ((token_class == RESERVED) && (sep_token->token_type == BASED)) {
            /* BASED <id>[ .<id> ] */
            /* Get based name */
            get_token_ptr(&token);
            token_class = get_token(token);
...
int parse_type(DECL_MEMBER *el_ptr, TOKEN *token) {
    ...
    if (token_class == LEFT_PAREN) {
        ...
        if ((token_class == NUMERIC) || ((token_class == OPERATOR) && (temp_token->token_type == TIMES))) {
    ...
    if ((token_class == RESERVED) && (token->token_type >= BYTE) && (token->token_type <= STRUCTURE)) {
        /* Save type token */
...
int get_element(DECL_MEMBER **element, TOKEN *token) {
    ...
    if (token_class == RESERVED) {
        switch (token->token_type) {
            case LABEL:
                ...
                token_class = get_token(token);
                if ((token_class == RESERVED) && ((token->token_type == PUBLIC) || (token->token_type == EXTERNAL))) {
                    /* Indeed, who cares? */
                    token_class = get_token(token);
                }
                return token_class;
            ...
            case LITERALLY:
                ...
                if (get_token(&temp_token) == END_OF_FILE) {
                    el_ptr->literal[0] = '\0';
                } else if (get_token(&eof_token) == END_OF_FILE) {
    ...
    if ((token->token_type == INITIAL) || (token->token_type == DATA)) {
